...
cd $script_dir
read -p "请输入安装绝对路径（如 /home/data）：" istdir

test ! -d ${istdir} && mkdir ${istdir}
...
istMysql () {
	#安装必要软件包
	yum -y install numactl libaio bzip2
	#创建相关目录
	blue "创建相关目录"
	mkdir -pv ${istdir}/mysql/{run,data,binlogs,log}
	...
	blue "解压并mv到指定目录"
	tar -xzvf mysql-5.7.32-linux-glibc2.12-x86_64.tar.gz && mv mysql-5.7.32-linux-glibc2.12-x86_64/* ${istdir}/mysql
	
	#添加环境变量
	blue "添加环境变量"
	echo "export PATH=$PATH:${istdir}/mysql/bin" >> /etc/profile
	source /etc/profile
	...
	#设置mysql文件夹权限
	blue "设置mysql文件夹权限"
	chown -R mysql.mysql ${istdir}/mysql/
	#chown -R mysql.mysql /data/mysql/
	
	#mysql的服务脚本放到系统服务中
	blue "mysql的服务脚本放到系统服务中"
	cp -a ${istdir}/mysql/support-files/mysql.server  /etc/init.d/mysqld
...
port = 3306
socket = /sqldir/mysql/run/mysql.sock

[mysqld] 
port = 3306 
socket      = /sqldir/mysql/run/mysql.sock
basedir = /sqldir/mysql                                                                                  
datadir = /sqldir/mysql/data                                                                             
pid-file = /sqldir/mysql/run/mysql.pid
innodb_file_per_table = ON                                                                                                                                              
log-error=/sqldir/mysql/log/mysql_error.log                                                              
...
#slow_query
slow_query_log=ON
slow_query_log_file=/sqldir/mysql/log/mysql_slow_query.log
long_query_time=2
...
#innodb_log_group_home_dir = /sqldir/mysql/redolog/
#innodb_undo_directory = /sqldir/mysql/undolog/
...
#binlog
log-bin=/sqldir/mysql/binlogs/mysql-bin.log
log-slave-updates = 1  
binlog-format=row
sync-master-info = 1
sync_binlog = 1 #ssd
...
report-port=33061
EOF
	totalmem=$(awk '/MemTotal/{total=$2}END{print (total)/1024/1024}'  /proc/meminfo)
	ibps=$(awk -v x=${totalmem} -v y=0.65 'BEGIN{printf "%.0f\n",x*y}')
	...
	sed -i 's#\/sqldir#'"$istdir"'#g' /etc/my.cnf
	...
	#初始化数据库
	blue "初始化数据库"
	cd ${istdir}/mysql/bin/ && ./mysqld --initialize --user=mysql --basedir=${istdir}/mysql/--datadir=${istdir}/mysql/data/
	
	#设置启动
	blue "设置启动"
	sleep 3
	\cp -fv ${istdir}/mysql/support-files/mysql.server /etc/init.d/mysql && chmod +x /etc/init.d/mysql
	...
	#临时密码
	blue "临时密码"
	DBPWD=$(grep 'temporary password' ${istdir}/mysql/log/mysql_error.log|awk '{print $NF}')
	echo "$DBPWD" > /tmp/pwtemp.txt && echo -e "\033[32m mysql临时密码为:$DBPWD 并且	密码保存在/tmp/pwtemp.txt。\033[0m"
	
	#mysql路径指定
	blue "mysql路径指定"
	ln -sfv ${istdir}/mysql/bin/mysql /usr/bin/mysql
}
