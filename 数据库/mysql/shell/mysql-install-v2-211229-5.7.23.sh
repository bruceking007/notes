...
blue "解压并mv到指定目录"
tar -xzvf mysql-5.7.23-linux-glibc2.12-x86_64.tar.gz && mv mysql-5.7.23-linux-glibc2.12-x86_64/* /data/mysql
...
insJem () {
	if test -d /usr/local/include/jemalloc;then
		...
		if test -f /usr/lib64/libjemalloc.so.1;then
			...
			red "请设置软链接 ln -s /usr/local/lib/libjemalloc.so.2 /usr/lib64/libjemalloc.so.1"
			exit
		fi
	fi
	...
	install () {
		cd $cur_path && tar -xf jemalloc-5.2.1.tar.bz2 && cd jemalloc-5.2.1
		./autogen.sh
		make -j4
		make install
