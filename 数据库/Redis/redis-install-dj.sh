...
mkdir -pv ${REDDIR}/{data,pid,logs,bin}
mkdir -pv ${REDDIR}/data/${REDPORT}

#cd ${REDDIR} && mv redis.conf redis-${REDPORT}.conf
cp -a /usr/local/redis-4.0.2/redis.conf ${REDDIR}/redis-${REDPORT}.conf
...
sed -i "s/^port.*/port ${REDPORT}/" ${REDDIR}/redis-${REDPORT}.conf
sed -i 's/^daemonize.*/daemonize yes/' ${REDDIR}/redis-${REDPORT}.conf
sed -i "s/^dir.*/dir \/usr\/local\/redis\/data/${REDPORT}" ${REDDIR}/redis-${REDPORT}.conf
sed -i "s/^pidfile.*/pidfile \/usr\/local\/redis\/pid\/redis-${REDPORT}.pid/" ${REDDIR}/redis-${REDPORT}.conf
sed -i "s/^logfile.*/logfile \/usr\/local\/redis\/logs\/redis-${REDPORT}.log/" ${REDDIR}/redis-${REDPORT}.conf
...
CONF="REDDIR/redis-REDPORT.conf"
case "$1" in
    start)
        if [ -f $PIDFILE ]
        then
...
cp_redis() {
echo -ne "\033[33m 请输入要部署redis集群的端口: \033[0m" && read REDPORT
mkdir -pv ${REDDIR}/data/${REDPORT}
cp -a /usr/local/redis-4.0.2/redis.conf ${REDDIR}/redis-${REDPORT}.conf
